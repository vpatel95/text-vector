...
namespace wordvec {
    trainer_t::trainer_t(const std::shared_ptr<train_setting_t> &_trainSettings,
        ...
        sharedData.expTable.reset(new std::vector<float>(_trainSettings->table_sz));
        for (uint16_t i = 0; i < _trainSettings->table_sz; ++i) {
            // Precompute the exp() table
            (*sharedData.expTable)[i] =
                    exp((i / static_cast<float>(_trainSettings->table_sz) * 2.0f - 1.0f)
                                           * _trainSettings->table_max);
            // Precompute f(x) = x / (x + 1)
            (*sharedData.expTable)[i] = (*sharedData.expTable)[i] / ((*sharedData.expTable)[i] + 1.0f);
        }

        if (_trainSettings->with_hs) {
